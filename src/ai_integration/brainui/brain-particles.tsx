...
export const BrainParticleMaterial = shaderMaterial(
  ...
  /*glsl*/ `
    ...
    void main() {
      ...
      gl_PointSize = randoms * 3.5 * (1. / -mvPosition.z);
    }
  `,
  // fragment shader
  /*glsl*/ `
...
export function BrainParticles({ curves }: { curves: THREE.CatmullRomCurve3[] }) {
  const density = 15; // Increased particle density
  ...
  const randoms = React.useMemo(() => {
    ...
    for (let i = 0; i < numberOfPoints; i++) {
      randoms.push(randomRange(0.4, 1));
    }
